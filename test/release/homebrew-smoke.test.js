...
test('stages downloaded candidate copies for formula and cask smoke', async () => {
  ...
  for (const target of RELEASE_TARGETS) {
    const filename = artifactName('0.1.0', target);
  ...
  const result = await stageHomebrewCandidate({
    record: {
      ...
      versions: { server: '0.1.0', desktop: '0.1.0' },
      artifacts,
    },
    releaseRoot,
    stagingRoot,
  ...
  const source = join(artifactsDirectory, 'portreeve-v0.1.0-macos-arm64');
  const staged = join(stagingRoot, 'v0.1.0-preview.1', 'portreeve-v0.1.0-macos-arm64');
  ...
  expect(result.formula).toContain('v0.1.0-preview.1/portreeve-v0.1.0-macos-arm64');
  expect(result.cask).toContain('v0.1.0-preview.1/PortReeve-0.1.0-macos-#{arch}.dmg');
});
