...
/** @param {unknown} descriptor @param {boolean} [announce] */
async function recordSelection(descriptor, announce = true) {
...
console.log('Option-click any element to select it without activating it.');
console.log(
  'Normal clicks use the live PortReeve service and can perform real actions.',
);
console.log(`Latest selection: ${selectionPath}`);
console.log('Commands: snapshot, selected, select <selector>, inspect <selector>,');
console.log(
...
async function runCommand(line) {
  ...
  switch (command) {
    ...
    case 'selected':
      try {
        ...
        if (/** @type {NodeJS.ErrnoException} */ (error).code === 'ENOENT') {
          console.log('No element has been selected yet.');
          return;
        }
        throw error;
      }
      ...
      await recordSelection(
        await page
          ...
          .evaluate((element) => {
            ...
            return inspectorWindow.__portreeveInspectorSelect(element, false);
          }),
        false,
      );
    ...
    case 'reload':
      ...
      return;
    case 'help':
      console.log('Option-click selects and outlines any renderer element.');
      console.log('snapshot                    print the accessibility snapshot');
      console.log('selected                    print the last Option-click selection');
      console.log('select <selector>           select and outline the first match');
      console.log(
