...
const workspaceMetadata = JSON.parse(await readFile(resolve('package.json'), 'utf8'));
const packageMetadata = JSON.parse(
  await readFile(resolve('packages', 'client', 'package.json'), 'utf8'),
);
if (
...
const packedClient = await packClient(releaseDirectory);
artifacts.push({
  type: 'npm-package',
...
async function packClient(destination) {
