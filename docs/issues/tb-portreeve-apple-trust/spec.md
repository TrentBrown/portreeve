...
- **AC5.** Complete native trust evidence. Independent native ARM64 and Intel
  ...
  application smoke results, and successful Gatekeeper assessments for the
  standalone CLI, the DMG's primary-signature open policy, and the mounted
  application. Aggregation rejects missing, malformed, stale, duplicate,
...
| R5 | Native Apple verification | Exactly one current native ARM64 and one current native Intel document contain every required identity, Apple, Gatekeeper, byte-equality, and smoke fact and pass strict aggregation. | Any architecture or required fact is absent, Rosetta substitutes for Intel, evidence is mutable, stale, duplicate, synthetic, cross-bound, or inconsistent, or exit status is accepted without parsed facts. | Native evidence documents, evidence-schema tests, aggregation positive and negative fixtures, exact command records, and hosted runner results. |
