# Decision Scratchpad - tb-portreeve-initial-release
...
## [1] Pin the Bun and JavaScript toolchain exactly

[-] **Promote**

**Confidence:** HIGH

**Blast Radius:** Local development, CI, dependency resolution, source
compatibility, and every compiled release artifact.

Pin Bun 1.3.14 and exact npm dependency versions rather than floating ranges.
Use Commander.js for CLI parsing, Zod at runtime schema boundaries, TypeScript
in no-emit `checkJs` mode, and ESLint/Prettier for source checks. CI and release
jobs must acquire the pinned Bun version explicitly.

The currently installed Bun 1.2.18 is not the project version and reports that
this Intel CPU needs a baseline build. Do not mutate the developer's global Bun
installation; use an isolated pinned baseline tool for local spike work.

**Triggered by:** P1 toolchain setup and the installed Bun AVX warning.

**Alternatives considered:**
- Float on current Bun/dependency releases - rejected because compiler/runtime
  drift would undermine standalone artifact reproducibility.
- Pin the installed Bun 1.2.18 - rejected because it is outdated and its local
  binary does not match this machine's CPU capability.

## [2] Reserve the unscoped `portreeve` name for the client package
...
## [3] Disable standalone executable environment autoload
...
## [4] Correct local tool architecture to native ARM64

[-] **Promote**

**Confidence:** HIGH

**Blast Radius:** Local dependency installation, compiled spike target,
toolchain diagnostics, and interpretation of the earlier Bun warning.

This host is Apple Silicon (`darwin/arm64`). The preexisting global Bun and the
first isolated baseline download were x86_64 processes running under
translation, while Node is native ARM64. Use Bun 1.3.14's verified
`bun-darwin-aarch64` asset for local development and spike execution. Retain
x64 baseline as a release target, not as this host's local tool.

This corrects scratchpad entry 1's inference that the machine itself was an
older Intel CPU. Its exact-version pin and do-not-modify-global-tool decisions
remain valid.

**Triggered by:** TypeScript installed an x64 platform package while native
ARM64 Node requested its ARM64 package; `uname`, Node, Bun, and `file` exposed
the mixed architectures.

**Alternatives considered:**
- Run the full toolchain under x86_64 translation - rejected because it hides
  native ARM64 compatibility and caused optional-package resolution conflicts.
- Replace the developer's global Bun installation - rejected because the
  project can use an isolated pinned tool without mutating global state.

## [5] Match Node development types to the native Node 24 toolchain

[-] **Promote**

**Confidence:** HIGH

**Blast Radius:** Static checking of the Node-compatible client and shared
JavaScript source.

Pin `@types/node` 24.13.3. It matches the native Node 24 development runtime
and avoids a TypeScript 7 failure caused by `@types/node` 26 importing the
untyped `punycode` JavaScript package from its declaration files. The
standalone CLI remains compiled by Bun, and the client keeps its documented
Node 22-or-newer runtime floor; source must not depend on APIs newer than that
floor without explicit compatibility tests.

**Triggered by:** The initial strict `checkJs` run failed only while traversing
third-party `@types/node` 26 declaration dependencies.

**Alternatives considered:**
- Disable or weaken JavaScript checking in dependencies - rejected because the
  failure can be avoided without reducing project checks.
- Downgrade TypeScript - rejected because the incompatibility is isolated to a
  type package targeting a newer Node line than the local development runtime.

## [6] Type-check against the minimum supported Node runtime
...
## [7] Preserve the invoking Bun runtime in nested project scripts
...
## [8] Define the protocol v1 envelope and exit-code bands
...
## [9] Make SQLite mutations and their audit events atomic
...
## [10] Establish conservative initial server-setting defaults
...
## [11] Keep the official client on the public Unix-socket protocol
...
## [12] Treat existing runtime paths as evidence, not cleanup targets
...
## [13] Serialize allocation races at the SQLite lease boundary
...
## [14] Expire ephemeral assignments separately from pending leases
...
## [15] Use composite process instances and fresh lineage for ownership
...
## [16] Derive every inventory view from one reconciliation snapshot
...
## [17] Bind reclamation to both live process instances and one confirmed run
...
## [18] Keep unsafe any-owner eviction operation-scoped and fingerprint-bound
...
## [19] Keep claim administration server-owned and listener-gated
...
## [20] Make prune consent a CLI concern while the server revalidates safety
...
## [21] Separate transactional audit history from best-effort diagnostics
...
## [22] Centralize stable CLI JSON errors and exit-code bands
...
## [23] Separate foreground execution from native per-user supervision
...
## [24] Make checksummed executables the release authority
...
## [25] Make process and supervisor inspection explicit across platforms
...
## [26] Require full source and supervisor gates on every native release target
...
## [27] Establish the first release identity and visibility gate
...
## [28] Consolidate the current reproducible toolchain contract
